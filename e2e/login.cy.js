describe('Проверка авторизации', function () {
    ...
    it('Верный логин и верный пароль', function () {
        ...
        it('Проверить "Забыли пароль"', function () {
            ...
            cy.get('#mailForgot').type('5709037@mail.ru'); // ввел любой адрес почты
        ...
        it('Верный логин и НЕверный пароль', function () {
            ...
            cy.get('#forgotEmailButton').should('have.css', 'color', 'rgb(0, 85, 152)'); // проверка слово "забыли пароль" (синего цвета)
            cy.get('#mail').type('german@dolnikov.ru'); // ввел верный логин
            cy.get('#pass').type('iLoveqastudio20'); // ввел НЕверный пароль
            cy.get('#loginButton').click(); // нажал войти
            cy.get('#messageHeader').should('be.visible');
        ...
        it('НЕверный логин и верный пароль', function () {
            cy.visit('https://login.qa.studio'); // Зайти на сайт
            cy.get('#forgotEmailButton').should('have.css', 'color', 'rgb(0, 85, 152)'); // проверка слово "забыли пароль" (синего цвета)
            cy.get('#mail').type('german@dolnikov12.ru'); // ввел НЕверный логин
            cy.get('#pass').type('iLoveqastudio1'); // ввел верный пароль
            cy.get('#loginButton').click(); // нажал войти
            cy.get('#messageHeader').should('be.visible');
            cy.get('#messageHeader').contains('Такого логина или пароля нет');
